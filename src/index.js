...
export default class RwserveLoremIpsum {
	...
	async processingSequence(workOrder) {
		try {
			if (this.customText != undefined) 
				workOrder.setOutgoingPayload(this.getCustomText());			
			else
				workOrder.setOutgoingPayload(this.getClassicText());
						
			workOrder.responseHeaders.setStdHeader('content-type', 'text/plain');
			workOrder.setStatusCode(200);
	...
	getClassicText() {				
		if (this.randomize == 'true') {
			...
			var output = [];
			for (let i=0; i < this.repeat; i++)
				output.push(phrases[phrases.length % i]);
			return output.join(' ');
		}
	}
	...
	getCustomText() {
		if (this.randomize == 'true') {
			var words = this.customText.split(' ');			
			var output = [];
			for (let i=0; i < this.repeat * words.length; i++) {
